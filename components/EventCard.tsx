...
export const EventCard = ({
  ...
  const {
    mutateAsync: toggleRemindersMutation,
    isPending: isRemindersPending,
  } = useToggleReminders();
  const { mutateAsync: editEventMutation, isPending: isSaving } =
    useEditEvent();
      ...
      <View className="flex-row justify-between items-start ">
        <View className="flex-row items-center gap-2">
          ...
          {isEditing ? (
            ...
            >
              <Text
                className={`text-[10px] font-bold uppercase tracking-wide mr-1 ${getBadgeStyles(editData.type).text}`}
              >
                {editData.type}
              </Text>
              <MaterialIcons
                name="loop"
                size={12}
                color={getBadgeStyles(editData.type)
                  .text.split(" ")[0]
                  .replace("text-", "")}
              />
            </TouchableOpacity>
            ...
            <View className={`px-2 py-0.5 rounded border ${badgeStyles.view}`}>
              <Text
                className={`text-[10px] font-bold uppercase tracking-wide ${badgeStyles.text}`}
              >
                {event.type}
              </Text>
            </View>
        ...
        {isEditing ? (
          <View className="flex-row items-center flex-1 gap-2">
            <TouchableOpacity
              ...
              className="flex-1 border-b border-gray-200 dark:border-slate-600 py-0.5"
            >
              <Text className="text-sm text-sky-600 dark:text-sky-400">
                {editData.date.toLocaleDateString(undefined, {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
                })}
            ...
            >
              <Text className="text-sm text-sky-600 dark:text-sky-400">
                {editData.date.toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </Text>
      ...
      {(isEditing ||
        ...
        <View className="mt-1">
          {isEditing ? (
            ...
            <Text
              numberOfLines={2}
              className="text-sm text-zinc-500 dark:text-zinc-400 leading-tight"
            >
              {event.description}
            </Text>
          )}
